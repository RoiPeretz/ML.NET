...
{
    Task<ImageDetectionResult[]> Detect(string fileName, ReadOnlyMemory<byte> image);
}
...
{
    public async Task<ImageDetectionResult[]> Detect(string fileName, ReadOnlyMemory<byte> image)
    {
        var tasks = clients.Select(wrapper => objectDetectionWorkflow.Detect(wrapper, fileName, image));
